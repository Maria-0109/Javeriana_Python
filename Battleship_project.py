#!/usr/bin/env python
"""This module contains the Battleship game, in which only one player is involved(single player mode),
as the enemy will be the computer(PC). 
The player has 10 ships that he places on his own size selected board. Whoever destroys all the opponent's ships will win. 
Plus, the player always has the first turn. For the simbols, the "D" represents a ship that has been destroyed,
meanwhile an "X" represents a missed shot.
"""
